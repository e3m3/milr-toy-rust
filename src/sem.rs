...
impl <'a> TypeCheck<'a> {
    ...
    fn check_binop(&mut self, ast: &dyn Ast) -> bool {
        todo!()
    }

    fn check_call(&mut self, ast: &dyn Ast) -> bool {
        todo!()
    }

    fn check_function(&mut self, ast: &dyn Ast) -> bool {
        todo!()
    }

    fn check_literal(&mut self, ast: &dyn Ast) -> bool {
        todo!()
    }

    fn check_module(&mut self, ast: &dyn Ast) -> bool {
        let expr: &ModuleExpr = match ast.get_kind() {
            ExprKind::Module(expr)   => expr,
            _                        => {
                eprintln!("Unexpected AST type: {}", ast);
    ...
    fn check_number(&mut self, ast: &dyn Ast) -> bool {
        todo!()
    }

    fn check_print(&mut self, ast: &dyn Ast) -> bool {
        todo!()
    }

    fn check_prototype(&mut self, ast: &dyn Ast) -> bool {
        todo!()
    }

    fn check_return(&mut self, ast: &dyn Ast) -> bool {
        todo!()
    }

    fn check_transpose(&mut self, ast: &dyn Ast) -> bool {
        todo!()
    }

    fn check_var(&mut self, ast: &dyn Ast) -> bool {
        todo!()
    }

    fn check_var_decl(&mut self, ast: &dyn Ast) -> bool {
        todo!()
    }
}

impl <'a> AstVisitor for TypeCheck<'a> {
    fn visit(&mut self, ast: &dyn Ast) -> bool {
        match ast.get_kind_id() {
...
impl <'a> DeclCheck<'a> {
    pub fn new(options: &'a RunOptions) -> Self {
        ...
        assert!(decl_check.scope.add_symbol(&expr_print.get_symbol().unwrap(), decl_check.options));
        assert!(decl_check.scope.add_symbol(&expr_transpose.get_symbol().unwrap(), decl_check.options));
        decl_check
    }

    fn check_binop(&mut self, ast: &dyn Ast) -> bool {
        let expr: &BinopExpr = ast.get_kind().to_binop().unwrap();
        expr.get_lhs().accept(self) && expr.get_rhs().accept(self)
    }

    fn check_call(&mut self, ast: &dyn Ast) -> bool {
        let expr: &CallExpr = ast.get_kind().to_call().unwrap();
    ...
    fn check_function(&mut self, ast: &dyn Ast) -> bool {
        let expr: &FunctionExpr = ast.get_kind().to_function().unwrap();
    ...
    fn check_module(&mut self, ast: &dyn Ast) -> bool {
        let expr: &ModuleExpr = ast.get_kind().to_module().unwrap();
        if self.options.is_verbose(VerboseMode::Sem) {
    ...
    fn check_print(&mut self, ast: &dyn Ast) -> bool {
        let expr: &PrintExpr = ast.get_kind().to_print().unwrap();
        let sym = ast.get_symbol().unwrap();
        if !self.scope.contains_symbol(&sym, self.options) {
    ...
    // TODO: Forward declarations
    fn check_prototype(&mut self, ast: &dyn Ast) -> bool {
        let expr: &PrototypeExpr = ast.get_kind().to_prototype().unwrap();
        for arg in expr.get_args().iter() {
            let sym = arg.get_symbol().unwrap();
            if self.scope.contains_symbol(&sym, self.options) {
    ...
    fn check_return(&mut self, ast: &dyn Ast) -> bool {
        let expr: &ReturnExpr = ast.get_kind().to_return().unwrap();
    ...
    fn check_transpose(&mut self, ast: &dyn Ast) -> bool {
        let expr: &TransposeExpr = ast.get_kind().to_transpose().unwrap();
        let sym = ast.get_symbol().unwrap();
        if !self.scope.contains_symbol(&sym, self.options) {
            eprintln!("Expected to find symbol '{}' in scope {}", sym, ast.get_loc());
            return false;
    ...
    fn check_var(&mut self, ast: &dyn Ast) -> bool {
        let _expr: &VarExpr = ast.get_kind().to_var().unwrap();
        let sym = ast.get_symbol().unwrap();
        if !self.scope.contains_symbol(&sym, self.options) {
            eprintln!("Expected to find symbol '{}' in scope {}", sym, ast.get_loc());
            false
    ...
    fn check_var_decl(&mut self, ast: &dyn Ast) -> bool {
        let expr: &VarDeclExpr = ast.get_kind().to_var_decl().unwrap();
...
impl Semantics {
    pub fn check_all(ast: &dyn Ast, options: &RunOptions) -> bool {
        ...
        let type_result: bool = ast.accept(&mut type_check);
        if !type_result {
            eprintln!("AST failed TypeCheck semantics check");
            exit(ExitCode::SemanticError);
        }
        if options.sem_exit { exit(ExitCode::Ok); }
        decl_result && type_result
    }
}
